...
(function ( $ ) {
    ...
    var LABEL_ELEMS = 'dateDropper:elems',
        LABEL_FUNCS = 'dateDropper:fns';

    /**
     * tracks an element to be removed on clean up
     * @param {Node|jQuery} elem
     * @param {Function} fn
     * @param {String} selector
     * @param {String} event_name
     */
    function add_fn(elem, fn, selector, event_name) {
        var fns = $(elem).data(LABEL_FUNCS) || [];
        fns.push([fn, selector, event_name]);
        $(elem).data(LABEL_FUNCS, fns);
    }
     ...
     * tracks an element to be removed on clean up
     * @param {Node|jQuery} elem
     * @param {Node|jQuery} extra
     */
    function add_elem(elem, extra) {
        var elems = $(elem).data(LABEL_ELEMS) || [];
        elems.push(extra);
        $(elem).data(LABEL_ELEMS, elems);
    }

    /**
     * removes elements created by dateDropper
     * @param {Node|jQuery} elem
     */
    function clean_up(elem) {
        var elems = $(elem).data(LABEL_ELEMS) || [],
            funcs = $(elem).data(LABEL_FUNCS) || [];

        $.each(elems, function (i, elem) {
            $(elem).remove();
        });

        $.each(funcs, function (i, handler) {
            var fn = handler[0],
                selector = handler[1],
                event_name = handler[2];

            $(selector).off(event_name, fn);
        });
    }
    ...
    $.fn.dateDropper = function( options ) {
        ...
        add_elem(this, $('<div class="dd_wrap" id="dd_'+drop_length+'"><div class="dd_overlay"></div><div class="dd_"></div></div>')
        .appendTo('body'));
